...
Only upright faces are vendored, matching what the hosted stylesheet served; italic is synthesised by the browser exactly as it was before, so the change is invisible on the page. Adding true italics would be a real improvement and a real visual change, and a separate decision.
